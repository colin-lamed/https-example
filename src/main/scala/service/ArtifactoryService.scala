...
class ArtifactoryService[F[_] : Async](implicit A: EnvAsk[F], F: Sync[F]) extends Http4sClientDsl[F] {
  ...
  def getRepos(implicit RA: ReqEnvAsk[F]): F[List[Repo]] =
    handleException("Artifactory.getRepos") {
      for {
        ...
        uri            =  Uri.unsafeFromString(s"$artifactoryUri/artifactory/v/1/repos")
