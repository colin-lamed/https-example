...
class HomeEndpoint[F[_] : EnvAsk : Sync](homeService: HomeService[F], dbService: DbService[F]) extends Http4sDsl[F] {
  ...
  def getHome(homeId: HomeId)(implicit RA: ReqEnvAsk[F]): F[Response[F]] =
    for {
