...
class RepoEndpoint[F[_] : Effect : EnvAsk : Sync](artifactoryService: ArtifactoryService[F]) extends Http4sDsl[F] {
  ...
  def getRepos(implicit RA: ReqEnvAsk[F]): F[org.http4s.Response[F]] =
   for {
  ...
  def getRepos2(implicit RA: ReqEnvAsk[F]): F[org.http4s.Response[F]] =
